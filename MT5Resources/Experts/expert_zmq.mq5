...
// Variables globales
string symbols[] = {"EURUSD", "GBPUSD", "USDJPY", "SPX500"};
string chartTF = "TICK"; // Hacer que chartTF sea una variable única en lugar de una lista
...
// Setting up error reporting
   ...
   // Iterar sobre los símbolos y suscribirse a ellos
for (int i = 0; i < ArraySize(symbols); i++) {
    string symbol = symbols[i];

    // Suscribirse al símbolo y marco de tiempo específico
    ...
    symbolSubscriptions[symbolSubscriptionCount].lastBar = 0; // inicializar con 0 para omitir el primer precio
    symbolSubscriptionCount++;

    // Imprimir mensaje de éxito en la consola
    string message = "Subscrito a " + symbol + " en el marco de tiempo " + chartTF;
    Print(message);
}
